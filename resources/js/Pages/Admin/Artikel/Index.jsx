      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="overflow-x-auto bg-white rounded-lg shadow-lg border border-gray-200">
          <table className="min-w-full text-sm text-left">
            ...
            <tbody>
              {(artikels || []).map((item) => (
                <tr key={item.id_artikel} className="border-t hover:bg-gray-50 transition">
                  <td className="px-6 py-4 font-medium text-gray-800">{item.judul}</td>
                  <td className="px-6 py-4 text-gray-600">
                    {item.kategori || <span className="italic text-gray-400">-</span>}
                  </td>
                  ...
                  <td className="px-6 py-4 text-gray-700">
                    {item.isi.length > 80 ? item.isi.slice(0, 80) + "..." : item.isi}
                  </td>
                  <td className="px-6 py-4">
                    {item.gambar ? (
                      <img
                  ...
                  <td className="px-6 py-4 space-x-2">
                    <Link
                      href={route("artikel.edit", item.id_artikel)}
